...
A-> read_csv() – Loads categorical data from a CSV file into a DataFrame.

B-> value_counts() – Counts the frequency of each unique category value.

C-> value_counts(normalize=True) – Returns percentage distribution of category values.

D-> crosstab() – Creates a cross-tabulation table to compare two categorical variables.

E-> groupby() – Groups dataset rows based on a selected categorical column.

F-> unique() – Returns all unique values present in a column.

G-> sort_values() – Sorts the dataset based on selected categorical values.

H-> filtering using condition – Extracts rows that satisfy a specific category condition.

I-> sum() – Adds values, often used after percentage or count operations.
